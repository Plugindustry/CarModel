...
public class TestBlock extends DummyBlock implements Tickable, EnergyInputable, EnergyOutputable, TexturedBlock, PistonPushable, PistonPullable {
    ...
    private TestBlock() {
        ...
        window.addWidget(new WidgetButton("button_1",
                ItemStackUtil.create(Material.OAK_SIGN).displayName("I'm button").getItemStack(),
                (pos, info) -> {
                    ...
                    MultiBlockManager.getAvailableStructures(this).stream()
    ...
    public boolean onInteract(@Nonnull Player player, @Nonnull Action action, @Nullable EquipmentSlot hand,
        ...
        if (super.onInteract(player, action, hand, tool, block, entity)) {
            if (action == Action.RIGHT_CLICK_BLOCK) player.openInventory(
                    ((TestBlockData) Objects.requireNonNull(MainManager.getBlockData(
    ...
    public static class TestBlockData extends BlockData {
        private transient final ExtendedInteractor<TestBlockData> interactor =
                new ExtendedInteractor<>(INSTANCE.window, this);
